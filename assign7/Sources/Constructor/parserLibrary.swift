...
protocol Table {
    ...
    func run() throws -> Int?
    func registerTable(rawTable: [Any])
}
...
class ScannerReadaheadTable: Table, TableWithTransitionsWithIntKey {
    ...
    func run() throws -> Int? {
        ...
        let character = scanner.peekInput()
        debug("\t peeked at `\(String (describing: character))`")
        let charAsInt =
            character == nil
            ? 256
            : Int(character!.asciiValue!)
        debug("\t\t with ascii `\(String (describing: charAsInt))`")
        let pair = transitions[charAsInt]
        debug(
            "\t\t transition for \(String (describing: charAsInt)) -> \(String (describing: pair)) "
        )
        if pair != nil {
...
class SemanticTable: Table {
    ...
    func registerTable(rawTable: [Any]) {
        ...
        (action, parameters, goto) = (
            rawTable[0] as! String, rawTable[1] as! [Any],
            rawTable[2] as! Int
        )
    }
    ...
    func run() throws -> Int? {
        ...
        if self.transducer!.canPerformAction(action) {
            ...
            error(
                "Semantic action \"\(action)\" needs to be added to "
        ...
        if let routine = parser.delayedRoutine {  //Run if not nil.
            ...
            if children.count == 0 {
                ...
                throw TransducerError.designError(
                    "incorrect reduce table; more than one child in tree")
            }
        }
...
public protocol Translator {
    ...
    func performAction(_: String, _: [Any])
}
...
public class Transducer: Translator {
    ...
    public func performAction(_: String, _: [Any]) {}
}
...
public final class Scanner: Transducer {
    ...
    public override func performAction(_ action: String, _ parameters: [Any]) {
        switch action {
        case "buildToken":
        ...
        for index in (0...rawTables.count - 1) {
            ...
            do {
                ...
                if tables.count != tableNumber {
                    error(
                        "Table numbered \(tableNumber) incorrectly inserted into table \(tables.count)."
                    )
                }
                tables.append(table)
            } catch {
...
public final class Parser: Transducer {
    ...
    init(sponsor: Translator?, parserTables: [Any], scannerTables: [Any]) {
        super.init()
        ...
        let keywords = (parserTables[0] as! [String]).dropFirst()
    ...
    private func registerTables(rawTables: [Any]) {
        for index in 0...rawTables.count - 1 {
            let rawTable = rawTables[index] as! [Any]
            let type = rawTable[0] as! String
            ...
            do {
                ...
                print("Sanitize Raw tables - error in your raw data")
                return
            }
        }
    }
    ...
    func buildTree(_ rootLabel: String) {
        ...
        let children =
            ...
            ? self.treeStack[self.left...self.right].compactMap { $0 } : []  // Picks up each non-nil object
    ...
    func buildTreeFromIndex(_ index: Int) {
        ...
        //Store it in instance newTree so a reduce table can use it
        let children: [any VirtualTree] =
            self.left <= self.right
            ? self.treeStack[self.left...self.right].compactMap { $0 } : []  // Picks up each non-nil object
        let label = self.tokenStack[index > 0 ? self.left + index - 1 : self.right + index + 1]
            .symbol
    ...
    func delayedBuildTreeFromIndex(_ index: Int) -> (() -> VirtualTree) {
        return {
            self.buildTreeFromIndex(index)
            return self.newTree!
        }
    }

    func discardScannerToken() {
