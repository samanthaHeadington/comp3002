...
protocol Relatable: Hashable, CustomStringConvertible {
...
class Relation<Item: Relatable, Relationship: Relatable>: CustomStringConvertible {
    ...
    func from(_ froms: [Item], relationsDo: (Relationship, Relation) -> Void) {
        ...
        let relations_from: [HashableTuple<Item, Relationship>] = triples.filter {
            froms.contains($0.from)
        }

        //partititions relations_from by relationship
        let from_map: [Relationship: [HashableTuple<Item, Relationship>]] =
            relations_from.partitionUsing { return $0.relationship }
        ...
        for mapping: (key: Relationship, value: [HashableTuple<Item, Relationship>]) in from_map {
            ...
            let subrelation: Relation<Item, Relationship> = Relation(
                from: mapping.value.map { ($0.from, $0.relationship, $0.to) })
    ...
    func performStar(_ items: [Item]) -> [Item] {
        var result: [Item] = [Item](items)

        var i = 0
        while i < result.count {
            from([result[i]]) { relationship, moreItems in
                result.appendIfAbsent(moreItems.allTo())
            }
            i += 1
        }
    ...
    static func example1() {
        ...
        var relation: Relation<Int, String> = Relation<Int, String>(from: [
            (2, "<", 3), (1, "=", 1),
            (3, ">", 1), (2, "<", 4), (1, "<", 5), (5, "<", 6), (2, "<", 5),
        ])
    ...
    static func example3() {
        ...
        print(
            #"Relation contains (2, "<", 3) is (true) "#,
            relation.triples.contains(HashableTuple<Int, String>((2, "<", 3))))
        print(
            #"Relation contains (2, "<", 1) is (false) "#,
            relation.triples.contains(HashableTuple<Int, String>((2, "<", 1))))
    ...
    static func example4() {
        var down: Relation<Int, String> = Relation<Int, String>(from: [
            (1, "L", 3), (2, "G", 5), (5, "A", 7), (2, "X", 7),
        ])
