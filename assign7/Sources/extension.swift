extension Array {
    ...
    mutating func mutatingDo(_ operation: (inout Element) -> Void) {
        var i = 0
        while i < count {
            operation(&self[i])
            i += 1
    ...
    mutating func appendIf(contentsOf: [Element], _ predicate: Bool) {
        if predicate {
            append(contentsOf: contentsOf)
        }
    }
    ...
    func collect<Into>(operation: ((Element) -> Into)) -> [Into] {
        var result: [Into] = []
        for elt: Element in self {
